...
ui <- fluidPage(
  ...
  # Application title
  titlePanel(" - My analyser - "),
  
  # Sidebar with a slider input for number of bins 
  sidebarLayout(
    sidebarPanel(width=4,h4("UPLOAD MODEL"),
                 fileInput("model_filename", "", accept = c(".RData")),
                 br(),
                 actionButton("",
                              ""),
                 textOutput(""),
...
server <- function(input, output) {
  ...
  dashboard_summary_model<-eventReactive(input$check_model_summary,{
    ...
    output_extra <- uploaded_model_list()$model_output[["extra..."]]
    
    
    output_extra_plot <- ggplot()
    
                            
    output <- list(output_verbatim = output_verbatim,
                   output_summary_table = output_summary_table,
                   output_extra_plot = output_extra_plot)
    return(output)
  })
  ...
  output$output_extra_plot <- renderPlot({dashboard_summary_model()[["output_extra_plot"]]})
  ...
  input_data_example <- reactive({
    ...
    return(list(vars_names = paste0(c("id",vars_input), collapse = ";"),
               row_example = paste0(c("01",row_input), collapse = ";")))
  ...
  get_predictions <- eventReactive(input$get_new_data,{
    ...
    # - add predictions column to the table
    current_test_data[["Predictions"]] <- as.factor(current_predictions)
    
    pred_plot <- ggplot()
    
    output <- list(pred_data = current_test_data, pred_plot = NULL, ok = "Finished")
    return(output)
  })
